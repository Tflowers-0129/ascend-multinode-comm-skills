...
如果用户仅要求完善技能/文档，不索要真实凭据、不连接历史服务器。现场任务只补齐本次必需的连接信息；用户明确只查宿主网络时，不索要容器、工作目录或部署脚本。角色、并行参数、版本等优先从远端发现，只有会改变结论的歧义才问用户。意图不明确且会影响测试范围时确认是预检还是排障；不因为给了脚本就判定服务有故障。

用户选择普通密码登录时直接使用可交互 SSH 会话，不强制其先准备密钥或独立核对首次指纹。默认采用正常的首次连接信任，保留后续主机密钥变化检查；不要为同一次普通登录反复索取确认。密码仅用于本次目标的认证交互，不写入命令行、脚本、配置或报告。用户要求后续免密时，在密码登录后创建本地专用密钥、仅向指定服务器追加公钥，并验证密钥登录；不覆盖已有密钥或关闭密码登录。具体步骤与能力限制见 [远端连接指引](references/remote-server-audit.md#2-建立-ssh-连接密码优先按用户选择)。
