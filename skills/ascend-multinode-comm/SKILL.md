...
description: 面向 Ascend/A5 与 vLLM-Ascend 的多机通信预检、部署脚本审计和分层排障。支持上传脚本分析，或收集服务器 IP、用户名、认证方式等信息后，通过 SSH 直连只读审计现场脚本与容器配置，定位影响建链的文件行号。主动检测覆盖 TCPStore/Gloo/HCCL、逐卡通信与拓扑，适用 PD 混部、分离及 KV 池化。
---
...
| 用户意图 | 行为 |
|---|---|
| 上传脚本、分析文件 | 本地只读审计，不自动连接服务器 |
| 连接服务器查看脚本、现场审计 | 读 [服务器直连审计](references/remote-server-audit.md)，收集连接信息后，通过已有 SSH/服务器连接能力只读查看并分析 |
| 验证通信、打流、检测 HCCL | 按下方主动预检流程，明确节点、空闲卡、端口与时间窗 |

如果用户仅要求把能力加入技能或文档，不向其索要真实凭据，也不发起连接。实际直连任务才收集服务器 IP/SSH 别名、端口、用户名、认证方式（密钥或密码）、脚本目录、容器名、节点角色和允许读取范围；已有信息不重复询问。密码认证需要用户通过可用的安全输入渠道提供登录密码，不要求贴入聊天；没有安全渠道时说明限制并让用户在自己的 SSH 终端输入或准备密钥登录，不编造自动密码接入能力。

直连审计可以读取现场脚本、容器挂载、限定日志和网卡/监听状态，但不执行部署脚本、source 环境脚本、导入项目 Python、拉起服务或做 TCP/HCCL 打流。SSH 登录成功不等于服务通信通过。取得现场证据后接入下方同一套审计流程，报告保留节点、容器、远端绝对路径、行号、采集时间和未验证项。

## 上传部署脚本：先做只读审计

用户要求“分析这套部署脚本”“找影响建链的错误”时，读 [部署脚本审计](references/deployment-script-audit.md)，进入静态模式，不直接启动下述主动预检。

从上传目录/文件列表，或已授权直连取得的现场文件，还原每个节点的入口、参数、source/调用链、namespace 与实际通信域。可用 `scripts/audit_deployment.py` 提取静态 shell 线索，再由助手读取工具未覆盖的 Python/Compose/K8s/复杂 shell 做语义分析；不要把未覆盖位置简单转交用户，也不要把扫描结果当最终结论。
