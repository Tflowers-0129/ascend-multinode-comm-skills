...
模板不记录密码值。用户选择密码时先正常 SSH 登录，不要求提前准备密钥或独立核对首次指纹；已有的本次凭据不要求重复发送。用户要求后续免密时，再创建本地专用密钥并追加公钥。密码仅用于认证，不写入命令参数、脚本或报告；详见 [服务器连接指引](remote-server-audit.md)。没有实际访问能力时如实说明，不能把本地推演当成已连接。
