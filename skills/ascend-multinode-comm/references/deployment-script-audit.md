# 多节点部署脚本审计：上传文件或服务器直连

## 输入与交付

可以上传整个目录、已解压的脚本包，或若干入口脚本及配套配置。优先保留目录结构、P/D/存储节点对应关系、实际启动参数、镜像/CANN/vLLM-Ascend 版本；缺少信息先分析已知部分，只询问会影响结论的问题。不要求用户先学会填写清单。

也可以不上传：用户要求连接服务器直接查看时，先读 [服务器直连审计](remote-server-audit.md)，按其中的信息模板收集 IP、端口、用户名、认证方式及密码的安全输入渠道、脚本路径和容器范围。完成授权范围内的只读采集后，继续本文的跨节点分析；不要把“没有上传文件”当作不能审计的理由。只收到上传文件不代表已授权远端连接。
