...
默认输入是指定服务器、容器、工作目录内的远端部署脚本。用户说“服务当前建链失败”时先按 [现场排障主流程](remote-server-audit.md) 连接服务器、关联日志和实际状态，本文用于其中的脚本语义核对；不能先要求用户上传文件或准备本地 manifest。远端相对路径按真实 cwd/namespace 解析，报告回指原始远端文件行号。
