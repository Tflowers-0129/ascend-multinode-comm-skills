...
优先复用现场官方教程，不先开发通用控制器。核心只有三步：加载 MPI/CANN 环境，按每节点 rank 数写 hostfile，然后直接用 mpirun 运行官方 hccl_test。
...
hostfile 的 `host:slots` 表示该节点启动多少 rank；例如两台机器各 8 卡就是两行 `:8`，总进程数 `-n 16`。先保证发起节点能免密 SSH 到 hostfile 中的节点。

按 Wiki 形式直接执行，路径和网卡替换为当前环境：

~~~bash
export PATH=/path/to/mpich/bin:$PATH
source /path/to/cann/set_env.sh
export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/path/to/cann/lib64:/path/to/mpich/lib
export HCCL_BUFFSIZE=2048
export HCCL_SOCKET_IFNAME=<两端实际通信网卡名>
# 若现场已有集群环境脚本，在这里 source；没有就跳过。

cd /path/to/hccl_test
mpirun -f hostfile -n <总rank数> ./bin/alltoall_test \
  -b 8 -e 1G -f 2 -a aiv
~~~

快速隔离单张卡时不要跑 16 rank：hostfile 改为每台 `:1`，设置 `HCCL_TEST_USE_DEVS=<device_id>`，用 `-n 2 -p 1` 跑 8 KiB Broadcast。先测健康卡，再只替换 device id 测故障卡，并加约 90 秒 timeout。健康卡成功、故障卡在打印测试参数后始终没有首条结果且 timeout=124，说明问题跟随该卡的跨机通信路径。

若发起节点不能通过自身管理 IP 免密 SSH，可把 hostfile 首行写为 `localhost:1`，同时给 Hydra 指定 `-iface <发起节点实际回连网卡>`；否则远端 proxy 可能错误回连自己的 localhost。普通 HCCL 测试用于定位通信路径，不能替代 MC2 算子支持性验证。

单机 EP/MC2 成功与某张卡的对外光口 DOWN 不矛盾：单机通信通常走机内 HCCS/UB 等路径，跨机才使用出机链路。只有单机 rank 组合也必须经过该物理端口、且确实让该 rank 产生非零通信量时，端口 DOWN 才应同时影响单机测试。

本次实操还确认了两点：容器能看到网卡但没有 `ip` 命令时，不能报成“网卡不存在”，直接使用镜像已有的 `ifconfig` 或跳过这类额外检查；教程中的集群环境脚本若现场不存在，就使用当前已验证的 CANN/MPI 环境，不为补齐教程路径而新增安装或包装层。
